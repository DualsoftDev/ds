...
namespace ChartAndGraph
{
    ...
    public class PieData : AbstractChartData, IInternalPieData , IChartData
    {
        ...
        /// custom RestoreRadiusScale   for DS Pilot   테스트 필요
        public void RestoreRadiusScale(string category, float radiusScale)
        {
            CategoryData current = (CategoryData)(mDataSource.Columns[category].UserData);
            current.RadiusScale = radiusScale;
            RaisePropertyUpdated();
        }


        public void RestoreColor(string category, Color color)
        {
            CategoryData current = (CategoryData)(mDataSource.Columns[category].UserData);
            //mDataSource.Columns[category].Material.SetColor("_Color", color);
            RaisePropertyUpdated();
        }

        public int GetCategoryIndex()
        {
            return mDataSource.Columns.Count;
        }
        ///
