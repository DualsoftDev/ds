...
|SEQ2|Reset Causal| \|> | A > B <\| C|Action B would be executed by causal action A, and the action B would be initialized(reset) by causal action C <p>`B행위는 A행위에 의해 실행되며, C행위애 의해 리셋됨`| ![AAA](./png/Seq2.dio.png)|
