            ...
            |> Seq.iter(fun sys ->
                ...
                for row in tableIO.Rows do
                    ...
                    then
                        ...
                        | XlsAutoModeLamp      -> updateLamp (row, LampType.DuAutoLamp       , tableIO)
                        | XlsManualModeLamp    -> updateLamp (row, LampType.DuManualLamp     , tableIO)
                        | XlsDriveModeLamp     -> updateLamp (row, LampType.DuDriveLamp      , tableIO)
                        | XlsStopModeLamp      -> updateLamp (row, LampType.DuStopLamp       , tableIO)
                        | XlsEmergencyModeLamp -> updateLamp (row, LampType.DuEmergencyLamp  , tableIO)
                        | XlsTestModeLamp      -> updateLamp (row, LampType.DuTestDriveLamp       , tableIO)
                        | XlsReadyModeLamp     -> updateLamp (row, LampType.DuReadyLamp      , tableIO)
