...
module CoreModule =
    ...
    type DsSystem with
        ...
        member x.AddLamp(lampType:LampType, lampName: string, flow:Flow) =
            if x <> flow.System 
            then failwithf $"button [{lampName}] in flow ({flow.System.Name} != {x.Name}) is not same system"
            x.Lamps.Add(LampDef(lampName, lampType, "", flow))
