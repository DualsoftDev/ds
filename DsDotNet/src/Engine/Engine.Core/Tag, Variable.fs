...
module TagVariableModule =
    ...
    let defaultStorageCreationParams value tagKind =
        assert( isInUnitTest() || RuntimeDS.TryGetSystem().IsSome)

        {
            ...
            Address = None
            Comment = None
            System = RuntimeDS.TryGetSystem() |? getNull<ISystem>()
            Target = if tagKind > -1 then RuntimeDS.TryGetSystem().Cast<IQualifiedNamed>() else None
            TagKind = tagKind
            IsGlobal = false
