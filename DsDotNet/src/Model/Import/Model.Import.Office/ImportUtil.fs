...
module ImportU =
    ...
    type ImportUtil =
        ...
        //real call alias  만들기
        [<Extension>]
        static member MakeSegment (doc:pptDoc, model:Model) =
            ...
            |> Seq.iter(fun node ->
                ...
                // { <ahn> 확인 요망
                match dicChildParent.ContainsKey(node), segOrg with
                | true, (:? Call as call) ->
                    let real = dicVertex.[dicChildParent.[node].Key] :?> Real
                    let alias = Alias.Create(node.Name, CallTarget call, Real real)
                    dicVertex.Add(node.Key, alias)
                | false, (:? Real as real) ->
                    let flow = dicFlow.[node.PageNum]
                    let alias = Alias.Create(node.Name, RealTarget real, Flow flow)
                    dicVertex.Add(node.Key, alias )
                | _, _ ->
                    failwith "ERROR"
                //if dicChildParent.ContainsKey(node) then
                ...
                //else
                //    let flow = dicFlow.[node.PageNum]
                //    let alias = Alias.Create(node.Name, RealTarget (segOrg:?>Real), Flow flow)
                //    dicVertex.Add(node.Key, alias )
