...
module Prelude =
    ...
    type Coil(tag:string) = 
        interface IExpressionTerminal with
            member x.ToText() = tag
            member x.Equals t = t :? Coil && tag = t.ToText()
        member x.ToText() = tag
    ...
    type RelayTag(tag:string) = 
        interface IExpressionTerminal with
            member x.ToText() = tag
            member x.Equals t = t :? RelayTag && tag = t.ToText()
    ...
    type MemoryOnOffCondition =
        ...
        with
            ...
            interface IExpressionTerminal with
                ...
                member x.Equals t = t :? MemoryOnOffCondition && x.ToText() = t.ToText()

    /// 신뢰 작업용 graph 의 vertex.  대략 Old.Dual.Core.Circle 에 해당
    and
        [<DebuggerDisplay("{ToText()}")>]
        QgVertex(name, parent) =
            ...
            member val Properties = preValues with get
            ...
            interface IVertex with
                ...
                member x.Properties = x.Properties :> IDictionary<VertexPropertyType, obj>
                member x.Name             = name
                member x.ToText()         = x.ToText()
    ...
    and
        ...
        QgPort(parent, portType:PortCategory, tagType:TagType option, plcTag:PLCTag list, plcFuncs:IFunctionCommand ResizeArray) =
            ...
            member val ConditionExpression = Expression.Zero with get, set
            member val InterlockExpression = Expression.Zero with get, set
            ...
            interface IPort with
                ...
                member x.ConditionExpression with get() = x.ConditionExpression and set(e) = x.ConditionExpression <- e
                member x.InterlockExpression with get() = x.InterlockExpression and set(e) = x.InterlockExpression <- e
    ...
    and QgSelect(name, parent) =
        ...
        member x.AddConditionDAG(key:IFunctionCommand list, dag:IDAG) =
            x.DAGs.Add(dag)
    ...
    type MemoryOnOffSpec =
        ...
        with
            ...
            interface IExpressionTerminal with
                member x.ToText() = x.ToText()
                member x.Equals t = t :? MemoryOnOffSpec && x.ToText() = t.ToText()
