...
module SystemManagerModule =
    ...
    type SystemManager (sys:DsSystem, stg:Storages)  =
            //시스템 Tag는 하위 시스템과 공유
        let dsSysBit    name = (if stg.ContainsKey(name) then stg[name] else sysTag stg  name  DuBOOL   ) :?> PlanVar<bool>
        let dsSysUint8  name = (if stg.ContainsKey(name) then stg[name] else sysTag stg  name  DuUINT8  ) :?> PlanVar<uint8>
        let dsSysUint16 name = (if stg.ContainsKey(name) then stg[name] else sysTag stg  name  DuUINT16 ) :?> PlanVar<uint16>
