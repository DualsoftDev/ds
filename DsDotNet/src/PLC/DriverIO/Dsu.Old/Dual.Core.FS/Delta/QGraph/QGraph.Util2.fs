...
module QgUtilModule2 =
    ...
    /// Model Status 생성
    let generateModelStatus (procinfos:ProcessResultInfo list) (opt:CodeGenerationOption) (resetlocks:seq<IVertex * (PortCategory * RungInfo)>) (asyncInfos:(IVertex * Expression) list) (modelVertex:IVertex)  =
        ...
        let procinfo = procinfos |> List.tryFind(fun inf -> inf.Workbook.Model.DAG.Vertices |> Seq.contains(modelVertex))
        let innerInfos = procinfos |> List.where(fun inf -> modelVertex.DAGs |> Seq.exists(fun dag -> edgeEqual dag.Edges inf.Workbook.Model.DAG.Edges))
        ...
        let parentGoing =
            ...
            | Some(p) -> p.GoingPort.GetTerminal() 
        ...
        let jobEnd = 
            let dagTerminalExpressions =
                ...
                |> Seq.map(fun dag -> 
                    ...
                    | None ->
                        dag.Vertices |> Seq.map(fun v -> v.SensorPort.GetTerminal()) |> Seq.reduce mkAnd
                )
        ...
        let start = 
            let incoming = 
                procinfo 
                |> Option.bind(fun inf -> generateVertexIncomingEdgesExpression opt inf.Workbook inf.Workbook.Id2RelayDic modelVertex |> Some) 
                |> Option.defaultValue Expression.Zero
            incoming <&&> getVertexOutputExpression opt modelVertex
        let reset = modelVertex.ResetPort.GetTerminal()
        ...
        let selectInterlock = 
            ...
            | Some(parent) ->
                ...
                | :? ISelect as ps ->
                    parent.DAGs 
                    |> Seq.collect(fun dag -> dag.Vertices) 
                    |> Seq.where(fun v -> v <> modelVertex) 
                    |> Seq.map(fun v -> v.ReadyPort.GetTerminal()) 
                    |> Seq.reduce mkAnd
                | _ -> Expression.Zero
            | _ -> Expression.Zero
        ...
        let going = 
            {
                defaultExpressionInfo with
                    Set        = [parentGoing; (mkNeg reset); t_r; start] |> Seq.reduce mkAnd
                    Interlock  = (mkNeg t_f) <&&> selectInterlock
        ...
        let homing = 
            ...
            {
                defaultExpressionInfo with
                    Set        = ([(mkNeg start); (mkNeg finishlock); (mkNeg asynclock); t_f; modelVertex.ResetPort.GetTerminal()] |> List.reduce mkAnd) |> mkOr (mkAnd (mkNeg t_f) (mkNeg t_g))
                    Interlock  = mkNeg t_r
        ...
        let finishToSensor =
            ...
            |> Seq.map(fun c -> 
                {
                    defaultExpressionInfo with
                        Set        = modelVertex.FinishPort.GetTerminal()
                        CoilOrigin = c |> mkTerminal |> RelayMarkerPath.CoilOriginTypeExt.toCoilOrigin
                }
            )
        ...
        |> Seq.mapi(fun i t -> 
            ...
            | Some(f) ->  
                monad{
                    ...
                    let set = setPort.GetTerminal() |> getTerminal |> Option.bind(fun t -> t |> toPulse |> Some) |> Option.defaultValue Expression.Zero
                    let reset = resetPort.GetTerminal()
                ...
                |> Seq.map(fun source ->
                    ...
                    let relayinfo = 
                        let set = 
                            ...
                            | None -> Expression.Zero
                            |> mkAnd (source.SensorPort.GetTerminal())
                        let reset = 
                            match target |> IVertexExt.GetSegmentType segments  with
                            | External -> target.StartPort.GetTerminal()
                            | Internal -> target.GoingPort.GetTerminal()

                        let fRisingFunc = CoilOutput.PulseCoilMode(Prelude.Coil(source.SensorPort.GetTerminal().ToText()+"_AsyncTemp")) :> IFunctionCommand
                ...
                let interlock =
                    [
                        if target.UseOutputInterlock then
                            yield target.ResetPort.GetTerminal() |> mkNeg // 출력 interlock

                        if target.UseOutputResetByWorkFinish then
                            yield generateOutputResetByFinish opt target   // 출력 완료 비접

                        yield target.StartPort.InterlockExpression
                    ]
                    |> List.tryReduce mkAnd
                    |> Option.defaultValue Expression.Zero
                let set = (relayinfo |> Seq.map(snd)) @@ externals |> Seq.reduce mkAnd <&&> target.StartPort.ConditionExpression
