...
namespace Dsu.Common.CS.LSIS.Graph
{
        ...
        where N : IEquatable<N>
    {
        ...
        public Graph(bool directed, bool allowsReflexivity)
        {
            ...
            Global.Logger?.Debug($"Creating {directional} Graph..");
        ...
        {
            Global.Logger?.Debug($"\tCreating graph with {src.m_Nodes.Count} nodes, {src.m_Edges.Count} edges.");
        ...
        public virtual void MergeGraph(Graph<N, E> other)
        {
            if (other == null)
            {
                Global.Logger?.Warn($"Skip merging null graph.");
                return;
            }

            Global.Logger?.Debug($"Merging graph with {other.m_Nodes.Count} nodes, {other.m_Edges.Count} edges.");
        ...
        public void AddEx(N node)
        {
            ...
            m_Nodes.Add(node);
            Global.Logger?.Debug($"Added node: {node}");
        }
        ...
        public void Add(N item)
        {
            ...
            {
                Global.Logger?.Warn($"Exception on Add:{ex}");
            }
        }
        ...
        public Edge<N, E> AddEdgeEx(N start, N end, E data)
        {
            ...
            Global.Logger?.Debug($"Adding edge: {start} -> {end} : {data}");
        ...
        public Nullable<Edge<N, E>> AddEdge(N start, N end, E data)
        {
            ...
            {
                Global.Logger?.Warn($"Exception on AddEdge:{ex}");
                return null;
            }
        }
        ...
        public Nullable<Edge<N, E>> GetEdge(N start, N end)
        {
            if (m_bDirected)
            {
                ...
                {
                    if (p.Start.Equals(start) && p.End.Equals(end))
                        return p;
                }
                return null;
            }
        ...
        public bool RemoveEdge(N start, N end)
        {
            for (int edge = m_Edges.Count - 1; edge >= 0; edge--)
            {
                ...
                {
                    Global.Logger?.Debug($"Removing Edge:{edge}");
        ...
        public void Clear()
        {
            Global.Logger?.Debug($"Clearing graph.");
            m_Nodes.Clear();
            m_Edges.Clear();
        }

        /// Determines if the graph contains the specified node
        /// <param name="item">The node to look for</param>
        /// <returns>True if the node is in the graph</returns>
        public bool Contains(N item) { return m_Nodes.Contains(item); }
        ...
        {
            if (!m_Nodes.Remove(item))
                return false;

            Global.Logger?.Debug($"Removing node: {item}.");
            for (int edge = m_Edges.Count - 1; edge >= 0; edge--)
            {
                if (m_Edges[edge].Start.Equals(item) || m_Edges[edge].End.Equals(item))
                    m_Edges.RemoveAt(edge);
            }
            return true;
        }
