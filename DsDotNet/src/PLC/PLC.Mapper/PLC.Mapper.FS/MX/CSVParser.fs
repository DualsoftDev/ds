...
module CSVParser =
    ...
    let rowProcessedEvent = Event<int>()
    let RowProcessed = rowProcessedEvent.Publish

    let totalLinesEvent = Event<int>()
    ...
    let parseCsv (filePath: string, skipRows: int) =
        ...
        for line in lines do
            let fields =
                splitLine(line)
                |> Array.map (fun s -> s.Trim().Trim('"')) // ← 쌍따옴표 및 공백 제거

            match rowCount with
            | n when n = skipRows ->
                columns <- Array.mapi (fun i name -> { ColumnName = name; Index = i }) fields
            | n when n > skipRows ->
                let row = Dictionary<string, string>(fields.Length)
            ...
            if rowCount % 100 = 0 || rowCount = totalLines then
                rowProcessedEvent.Trigger(rowCount)
    ...
    let readCommentCSV (files: string seq) =
        let readCommentCSV (file: string) =
            ...
            seq {
                for row in data.Rows do
                    let name = row.Values.ToArray() |> Seq.head
                    let comment = row.Values.ToArray()|> Seq.last
                    yield name, comment 
            }
    ...
    let readGlobalLabelCSV (files: string seq) =
        let readCSV (file: string) =
            ...
            seq {
                for row in data.Rows do
                    let values = row.Values |> Seq.toArray
                    if values.Length > 2 then
                        let name = values.[1]
                        let dataType = values.[2]
                        yield name, dataType
            }
