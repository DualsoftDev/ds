// http://www.fssnip.net/7Va/title/Await-Computation-Expression-Task-Async-Interoperability
/// Await Computation Expression (Task + Async Interoperability)
module Old.Dual.Common.AwaitableBuilder
...
module Awaitable =
    ...
    let using (a: 'a :> IDisposable) (f: 'a -> Awaitable<_>) =
        let dispose =
            let mutable flag = 0
            fun () ->
                if Interlocked.CompareExchange(&flag, 1, 0) = 0 && a |> box <> null
