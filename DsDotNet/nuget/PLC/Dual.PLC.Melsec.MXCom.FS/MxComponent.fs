...
module MxComponentModule  =
    ...
    type PlcMxComponent(logicalStationNumber: int) =
        ...
        do 
            ...
            if 0 <> ret
            then 
                match errorMessages.TryGetValue(ret) with
                | true, message -> failwithf "0x%08X [HEX] - %s" ret message
                | _ -> failwith "Failed to open the connection to the PLC"// sprintf "0x%08X [HEX] - 알 수 없는 오류" ret
