...
type WriterDBSettings(sqlitePath:string, dbWriter:string, modelFilePath:string, syncIntervalMilliSeconds:int) =
    do
        noop()
    new() = WriterDBSettings("", "", "", 0)
    [<JsonIgnore>]
...
[<AllowNullLiteral>]
type DSCommonAppSettings(loggerDBSettings:WriterDBSettings) =
    do
    ...
    member val WriterDBSettings = loggerDBSettings with get, set
