...
namespace DSModeler
{
    ...
    public partial class GroupDetailPage : XtraUserControl
    {
        ...
        public GroupDetailPage(DsHMIDataFlow dataGroup, PageGroup child)
        {
            InitializeComponent();
        ...
        void CreateLayout(DsHMIDataFlow dataGroup, PageGroup child)
        {
            for (int i = 0; i < dataGroup.Items.Count; i++)
                CreateLayoutCore(dataGroup.Items[i], child, i);
        }
        void CreateLayoutCore(DsHMIDataCommon item, PageGroup child, int index)
        {
