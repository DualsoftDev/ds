...
|Id| Item | Unit | Example| Desc |  GUI |
|:---:|:----|:-------:|:---:|:----|:---|
|OP1|End  Value | ( ) | `(Seg), A > B`  | Basically, actions placed in parentheses () are treated as end values<p>Action B would be executed by causal action A and the value of action SEG(end port of segment) is 'True'<p>`기본적으로 괄호() 안에 쓰여진 행위는 값으로 취급한다`<p>`행위 B는 행위 A가 실행되고, 행위 SEG의 end port 값이 'True'일 때 수행`    |![AAA](./png/Op1.dio.png)|
...
|OP19| XOR | #xor( , ) | #xor(B, C) > A | A would be executed when the exclusive or case (B end, C end) is true |![AAA](./png/Op19.dio.png)|
