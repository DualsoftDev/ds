namespace Old.Dual.Core.QGraph
...
module ExpressionGenerator =
    ...
    let getVertexSensorExpression (opt:CodeGenerationOption) (v:IVertex) = 
        match v.SensorPort.GetTerminal(), opt.SensorTagGenerator with
    ...
    let getVertexOutputExpression (opt:CodeGenerationOption) (v:IVertex) =
        match v.StartPort.GetTerminal(), opt.SensorTagGenerator with
        | Expression.Zero, Some(f) -> f v |> toCoil
        | Expression.Zero, None -> v.ToText() |> toCoil
        | _ as expr, _ -> expr
    ...
    /// 
    let generateOutputInterlocks opt (wookbooks:RelayMarkerWorkbook list) (v:IVertex) =
        if v.isSelfReset() then v.SensorPort.GetTerminal()
        else
            ...
            |> List.map (fun p -> 
                let pvIncoming =
                    monad{
                        let! wb = wookbooks |> Seq.tryFind(fun wb -> wb.Model.DAG.Vertices |> Seq.contains(p.Parent))
                        p.Parent |> generateVertexIncomingEdgesExpression opt wb wb.Id2RelayDic 
                    }
                    |> Option.defaultValue Expression.Zero
                
                pvIncoming <&&> p.GetTerminal())
            |> List.tryReduce mkAnd
            |> Option.defaultValue Expression.Zero
        <&&> v.ResetPort.ConditionExpression
    ...
    let generatePrecedeResetTargetExpression (wb:RelayMarkerWorkbook) source = 
        monad{
            ...
            let! result =
                precede 
                |> Seq.map(fun v -> v.SensorPort.GetTerminal()) 
                |> Seq.tryReduce mkAnd 
    ...
    let generateLadderInfo (modelComments:string seq) (opt:CodeGenerationOption) wb id2RelayDic =
            ...
            |> List.map (fun (name, vs) ->
                ...
                let manual = 
                    let 원점 =
                        ...
                        | _ -> Expression.Zero
                    let manualTag = v.ManualTag |> Option.bind(fun tag -> tag |> mkTerminal |> mkAnd ("MANU_MODE_M" |> toCoil) |> Some) |> Option.defaultValue Expression.Zero
                    (*원점 <||>*) 
                    manualTag
                /// 자신의 Start(출력)
                let selfHold =
                    if v.UseSelfHold then
                        v.StartPort.GetTerminal()
                    else                        
                        Expression.Zero
                ...
                let condition =
                    ...
                    |> Seq.map (fun v ->
                        ...
                        generateVertexIncomingEdgesExpression opt wb id2RelayDic v <&&> forBlockFirst <&&> v.StartPort.ConditionExpression
                        )
                    |> Seq.reduce mkOr
        ...
        let relayRungInfos =
            let interlockRelay name = name + "_ITL" |> toCoil
            let relayOnOffConditionToExpression onOffCond r =
                ...
                | TrustedNodeStartCondition(v) -> 
                    ...
                    let blockFirst =
                        ...
                        |> Option.bind(fun v -> 
                            if v.SensorPort.GetTerminal() = cause then
                                Expression.Zero
                            else
                                v.SensorPort.GetTerminal() |> mkNeg 
                            |> Some)
                        |> Option.defaultValue Expression.Zero
                    blockFirst <&&> incoming <&&> cause <&&> forLastMoved <&&> forFirstMoved 
                | TrustedEdgeCondition(e)      -> 
                    ...
                    let interlockRelay = 
                        match r.RelayMarker.DuId with
                        | Unique(_) -> interlockRelay r.Name
                        | _ -> Expression.Zero
                    edge <&&> sensor <||> interlockRelay
            ...
            let modelResetExpression = 
                ...
                let resets = v.Parent |> Option.bind(fun parent -> parent.HomingPort.GetTerminal() |> Some) |> Option.defaultValue Expression.Zero
            ...
            |> List.collect (fun r ->
                ...
                let sets   = [ relayOnOffConditionToExpression r.Set r] |> ResizeArray
                let resets = [ relayOnOffConditionToExpression r.Reset r]|> ResizeArray

                /// Unique Relay 인터락 관련 expression
                let uniqueInterlock = interlockRelay r.Name
                let interlockSet = resets |> Seq.reduce mkOr |> remove (interlockRelay r.Name)
                let interlockReset = r.RelayMarker.Location.SensorPort.GetTag() |> Seq.map(mkTerminal >> mkNeg) |> Seq.reduce mkAnd
                //if isTerminal then
                ...
                [
                    yield Some { defaultExpressionInfo with Set = set; Reset = reset; Selfhold = y; CoilOrigin = Relay r; Comments = comments |> List.ofSeq }
                    yield 
                        match r.RelayMarker.DuId with
                        | Unique(_) -> Some { defaultExpressionInfo with Set = interlockSet; Reset = interlockReset; Selfhold = uniqueInterlock; CoilOrigin = uniqueInterlock.toCoilOrigin(); Comments = comments |> List.ofSeq }
                        | _ -> None
                ]
                |> List.choose id
        ...
        let rungs = vertexRungInfos @ relayRungInfos ///usedRelayRungInfos
