...
module CounterTestModule =
    ...
    type CounterTest() =
        ...
        member __.``CTU creation test`` () =
            ...
            let ctu = Counter.CreateCTU("myCTU", condition, 100us)
            ctu.OV.Value === false
            ctu.UN.Value === false
            ctu.DN.Value === false
            ctu.PRE.Value === 100us
        ...
        member __.``CTU with reset creation test`` () =
            let t1 = PlcTag.Create("my_counter_control_tag", false)
            let resetTag = PlcTag.Create("my_counter_reset_tag", false)
            let condition = tag t1
            let reset = tag resetTag
            let ctu = Counter.CreateCTU("myCTU", condition, reset, 100us)
            ctu.OV.Value === false
            ctu.UN.Value === false
            ctu.DN.Value === false
            ctu.RES.Value === false
