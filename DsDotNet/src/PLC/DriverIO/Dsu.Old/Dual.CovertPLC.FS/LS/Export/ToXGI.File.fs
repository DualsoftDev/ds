...
open Old.Dual.ConvertPLC.FS
open Old.Dual.ConvertPLC.FS.LsXGI.Config.POU.Program.LDRoutine
open Old.Dual.Common
...
module internal File =
    ...
    let rungInfoToStatement (opt:CodeGenerationOption) (gri:(IExpressionTerminal * seq<RungInfo>)) = 
        let merge = 
            let set = snd gri |> Seq.map(fun ri -> ri.Set <&&> ri.Start) |> Seq.reduce mkOr
            let reset = snd gri |> Seq.map(fun ri -> ri.Reset) |> Seq.reduce mkAnd
            let interlock = snd gri |> Seq.map(fun ri -> ri.Interlock) |> Seq.reduce mkAnd
            let manual = snd gri |> Seq.map(fun ri -> ri.Manual) |> Seq.head
            let selfhold = snd gri |> Seq.map(fun ri -> ri.Selfhold) |> Seq.head
            let comment = snd gri |> Seq.map(fun ri -> ri.Comments) |> Seq.head 
            let coil = snd gri |> Seq.map(fun ri -> ri.CoilOrigin) |> Seq.head
            { 
        ...
        let condition = merge |> rungInfoToExpr
    ...
    let generateXGIXmlFromStatement (prologComments:string seq) (statements:Statement seq) (tags:PLCTag seq) (unusedTags:PLCTag seq) (existingLSISprj:string option) =
        ...
        let manager =
            ...
            let hwconfs =
                ...
                [
                    ...
                    MMemoryConfig(Memory.W, 640*1024*8) |> m     // 640KB memory
                ]
        ...
        let generators =
            [
                ...
                "M",  fun () -> manager.AllocateTag(Memory.M, Size.X) |> Option.get
