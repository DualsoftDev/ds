...
// http://www.codeproject.com/Articles/7684/Using-C-Generics-to-implement-a-Cache-collection


namespace Dual.Common.Utils
{
    public delegate void CachePurgingEventHandler(object sender, object Key, object Value);
		...
		where Value: ICacheable
	{
        ...
        public event CachePurgingEventHandler PurgingHook;

        /// <summary>
        /// Cache 에서 공간 부족 등의 이유로 item 이 purge 될 때의 작업 지정
        /// </summary>
        /// <param name="key"></param>
        /// <param name="value"></param>
	    public void NotifyPurging(Key key, Value value)
	    {
	        var handler = PurgingHook;
	        if (handler != null)
		...
		public void Add(Key k, Value v)
       {
		...
		public void PurgeAll()
		{
            ...
            if ( cacheStore != null )
            {
                ...
                cacheStore.ForEach(pr => NotifyPurging(pr.Key, pr.Value));
                cacheStore.Clear();
            }
		}
        ...
        /// <summary> Finalizer 에 의한 호출.  reference counter 값이 0 된 후에, GC 에 의해서 호출됨 </summary>
        ~Cache()
        {
