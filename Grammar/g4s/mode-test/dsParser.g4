...
model: (system|properties|comment|codeBlock)* EOF;        // importStatement|cpus

//codeBlock:  '[' 'code' ']' '=' LBRACE (varDecl*) RBRACE;   //  |funDef*
codeBlock:  (varDecl | functionDecl)+;
    varDecl:   type IDENTIFIER (CODE_EQUAL expr)? CODE_SEMICOLON;
    type:   INTTYPE | WORDTYPE | FLOATTYPE | VOIDTYPE;

    // functionDecl: type IDENTIFIER '(' formalParameters? ')' block; // "void f(int x) {...}"
    functionDecl: type IDENTIFIER CODE_LPARENTHESIS formalParameters? CODE_RPARENTHESIS block; // "void f(int x) {...}"
    ...
    block:  CODE_LBRACE stat* CODE_RBRACE ;   // possibly empty statement block
    stat:   block
        |   varDecl
        |   'if' expr 'then' stat ('else' stat)?
        |   'return' expr? CODE_SEMICOLON 
        |   expr CODE_EQUAL expr CODE_SEMICOLON // assignment
        |   expr CODE_SEMICOLON          // func call
        ;

    expr:   funCall
        |   IDENTIFIER '[' expr ']'         // array index like a[i], a[i][j]
        |   CODE_MINUS expr                // unary minus
        |   '!' expr                // boolean not
        |   expr '*' expr
        |   expr (CODE_PLUS | CODE_MINUS) expr
        |   expr '==' expr          // equality comparison (lowest priority op)
        |   IDENTIFIER                      // variable reference
        |   INT
        |   CODE_LPARENTHESIS expr CODE_RPARENTHESIS
        ;
        funCall: IDENTIFIER CODE_LPARENTHESIS exprList? CODE_RPARENTHESIS;    // func call like f(), f(x), f(1,2);
    exprList : expr (',' expr)* ;   // arg list


system: '[' SYS ((IP|HOST) EQ host)? ']' systemName EQ (sysBlock|sysCopySpec);    // [sys] Seg = {..}
    sysBlock
...
layouts: '[' 'layouts' ']' (identifier1)? EQ layoutsBlock;
    layoutsBlock
        : LBRACE (positionDef)* RBRACE
        ;
positionDef: apiPath EQ xywh;
    apiPath: identifier2;
...
addresses: '[' 'addresses' ']' (identifier12)? EQ addressesBlock;
addressesBlock
    : LBRACE (addressDef)* RBRACE
    ;
addressDef: segmentPath EQ address;
...
interfaces
    : '[' 'interfaces' ']' (identifier1)? EQ LBRACE (interfaceListing)* RBRACE;
    interfaceListing: (interfaceDef (SEIMCOLON)?) | interfaceResetDef;
    ...
    interfaceResetDef: identifier1 (causalOperatorReset identifier1)+ (SEIMCOLON)?;


alias: '[' 'aliases' ']' (identifier1)? EQ LBRACE (aliasListing)* RBRACE;
    aliasListing:
        aliasDef EQ LBRACE (aliasMnemonic)? ( SEIMCOLON aliasMnemonic)* (SEIMCOLON)+ RBRACE
        ;
