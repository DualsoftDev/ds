...
module xlsTestModule =
    ...
    let check (systemRepo:ShareableSystemRepository, system:DsSystem, viewNodes:ViewNode seq) =
        let modelText =  system.ToDsText()
        let libdir = @$"{__SOURCE_DIRECTORY__}\\xls"
        let helper = ModelParser.ParseFromString2(modelText, ParserOptions.Create4Runtime(systemRepo, libdir, "localhost", None))
    ...
    type XLSTest() =
        ...
        let system, views = ImportM.FromPPTX systemRepo $"{__SOURCE_DIRECTORY__}\\ppt\\T5_Call.pptx"
