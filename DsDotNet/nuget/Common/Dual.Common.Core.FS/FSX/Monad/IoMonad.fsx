// https://www.youtube.com/watch?v=h00DRlHewrM
...
module mod2=
    ...
    type IoBuilder() =
        member _.Bind(io: Io<'a>, f: 'a -> Io<'b>): Io<'b> =
            fun eff -> f (io eff) eff

        member _.Return(x: 'a): Io<'a> = fun _ -> x

        member _.ReturnFrom(x: Io<'a>): Io<'a> = x

        member _.Zero(): Io<unit> = fun _ -> ()

    let io = IoBuilder()

    let readLine () : Io<string> = fun eff -> eff.ReadLine ()
    let printLine (text: string) : Io<unit> = fun eff -> eff.PrintLine text

    let rec readMoveIo letter =
      io {
        let! line = readLine ()
        match parseMove line with
        | Some position -> return { At=position; Place=letter }
        | None ->
            do! printLine "Bad move! Please input row and column numbers"
            return! readMoveIo letter
      }

    let rec nextMoveIo board letter =
      io {
        let! move = readMoveIo letter
        match makeMove board move with
        | Some newBoard -> return newBoard
        | None ->
            do! printLine "Bad move! Position is occupied."
            return! nextMoveIo board letter
      }

    let rec playIo { Board=board; WhoseTurn=currentPlayer } =
      io {
        do! printLine (sprintf "%A turn" currentPlayer)
        do! printLine (sprintf "%s" (render board))
        do! printLine ""
        ...
        match outcome newBoard with
        | Winner letter ->
            do! printLine (sprintf "%A wins!!!" letter)
            do! printLine (sprintf "%s" (render newBoard))
        | Draw ->
            do! printLine "It's a draw!"
        | NoneYet ->
            return! playIo { Board=newBoard; WhoseTurn=otherPlayer currentPlayer }
      }
