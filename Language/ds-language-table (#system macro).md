...
|OP3|End Latch Value | #latch(,)| `#latch(#(SEGA), #g(B)) > B;` | #latch (SEGA, #g(B)) is set to 'True' when the end port value of the action SEGA becomes 'True', and is maintained regardless of the change in the action SEGA thereafter.<p>Action B is executed when the value of #latch (SEGA, #g(B)) is 'True', and resets the value of #latch (SEGA, #g(B)) during execution by user set #g(B)<p>`#latch (SEGA, #g(B))는 행위 SEGA의 end port 값이 'True'가 되면 'True'로 설정되며, 이후 행위 SEGA의 변화와 무관하게 유지됨`<p>`행위 B는 #latch (SEGA, #g(B))의 값이 'True'일 때 실행되며, '사용자가 설정한 #g(B)에 의해' 실행중에 #latch (SEGA)의 값을 리셋함`  |![AAA](./png/Op3.dio.png)|
