...
|SEQ12| resetStart | \|>> |  A \|>> B <p>is equal to A > B ; A \|> B| Action B would be executed by end of action A<p>And also B would be initialized(reset) by executing action A<p>`B행위는 A행위에 의해 실행되며, A행위의 실행중에 리셋됨` | ![AAA](./png/Seq12.dio.png)|
