...
module FlowManagerModule =
    ...
    type FlowManager (f:Flow)  =
        ...
        let f_auto_btn       = cpv $"auto_{fn}"    FlowTag.auto_btn       
        let f_manual_btn     = cpv $"manual_{fn}"  FlowTag.manual_btn     
        let f_drive_btn      = cpv $"drive_{fn}"   FlowTag.drive_btn
        let f_stop_btn       = cpv $"stop_{fn}"    FlowTag.stop_btn
        let f_ready_btn      = cpv $"ready_{fn}"   FlowTag.ready_btn
        let f_clear_btn      = cpv $"clear_{fn}"   FlowTag.clear_btn
        let f_emg_btn        = cpv $"emg_{fn}"     FlowTag.emg_btn
        let f_test_btn       = cpv $"test_{fn}"    FlowTag.test_btn
        let f_home_btn       = cpv $"home_{fn}"    FlowTag.home_btn

        let f_auto_lamp       = cpv $"auto_{fn}"    FlowTag.auto_lamp       
        let f_manual_lamp     = cpv $"manual_{fn}"  FlowTag.manual_lamp     
        let f_drive_lamp      = cpv $"drive_{fn}"   FlowTag.drive_lamp
        let f_stop_lamp       = cpv $"stop_{fn}"    FlowTag.stop_lamp
        let f_ready_lamp      = cpv $"ready_{fn}"   FlowTag.ready_lamp
        let f_clear_lamp      = cpv $"clear_{fn}"   FlowTag.clear_lamp
        let f_emg_lamp        = cpv $"emg_{fn}"     FlowTag.emg_lamp
        let f_test_lamp       = cpv $"test_{fn}"    FlowTag.test_lamp
        let f_home_lamp       = cpv $"home_{fn}"    FlowTag.home_lamp
