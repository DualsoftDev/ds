...
[<AutoOpen>]
module OptionExtensionModule =
    [<Extension>]
    type OptionExtension =
        /// 없거나 (None 이거나), 있다면, 그것이 특정 조건을 만족해야 한다.
        [<Extension>]
        static member IsNoneOr<'a> (xo:'a option, pred:'a -> bool) =
            match xo with
            | Some x -> pred(x)
            | None -> true
        ...
        /// 있으면서 특정 조건을 만족해야 한다.
        [<Extension>]
        static member Exists<'a> (xo:'a option, pred:'a -> bool) = xo |> Option.exists pred

        [<Extension>] static member Iter(x, pred) = x |> Option.iter pred
        [<Extension>] static member IsSomeTrue(x) = x |> Option.exists id
        [<Extension>] static member IsSomeFalse(x) = x |> Option.exists id |> not
