...
module XGITag = //IEC61131Tag =
    ...
    let createSymbol name comment device kind  address plctype devicePos addressIEC=
        {Name=name; Comment=comment; Device=device; Kind = kind; Type=plctype; State=0; Address=address; DevicePos=devicePos;}

    let copyLocal2GlobalSymbol (s:SymbolInfo) =
        {Name=s.Name; Comment=s.Comment; Device=s.Device; Kind = ((int)Variable.Kind.VAR_GLOBAL); Type=s.Type; State=0; Address=s.Address; DevicePos=s.DevicePos; }
