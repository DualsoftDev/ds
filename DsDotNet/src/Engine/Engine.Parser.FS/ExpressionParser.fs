        ...
        let optStatement =
            ...
            | :? AssignContext as assignCtx ->
                ...
                | (:? RisingAssignContext as ctx)::[] ->
                    let risingCoil:RisingCoil = {Storage = storage; HistoryFlag = HistoryFlag(); System = RuntimeDS.System}
                    Some <| DuAssign (createExp ctx, risingCoil)
                | (:? FallingAssignContext as ctx)::[] ->
                    let fallingCoil:FallingCoil = {Storage = storage; HistoryFlag = HistoryFlag(); System = RuntimeDS.System}
