...
namespace Engine.Export.Office
{
    public static class OpenXmlPPT
    {
        ...
        public static string ExportPPT(DsSystem sys, string templateFile, string targetFile)
        {
            ...
            var ppt = PresentationDocument.Open(targetFile, false);

            //첫페이지 이름을 시스템이름으로 변경 
            UpdateFirstSlide(ppt.PresentationPart, sys.Name);
            // 슬라이드 추가
        //    AddSlidesWithNames(ppt.PresentationPart, sys.Flows);
        ...
        private static void AddSlidesWithNames(PresentationPart presentationPart, HashSet<Flow> flows)
        {
            throw new NotImplementedException();
        }

        private static void UpdateFirstSlide(PresentationPart presentationPart, string name)
        {
        ...
        //        shape, page, geometry, shape.IsDashShape())
            ...
            if (presentationPart.SlideParts.Count() > 0)
            {
                // 첫 번째 슬라이드의 제목을 주어진 이름으로 설정
                var firstSlide = presentationPart.SlideParts.First();
                var firstShape = firstSlide.Slide.CommonSlideData.ShapeTree.Descendants<Shape>().FirstOrDefault();
                if (firstShape == null)
                    throw new Exception("Presentation first slide firstShape is not exist");


                var shapeProperties = firstShape.Descendants<NonVisualShapeProperties>().FirstOrDefault();
                var textProp = shapeProperties.Descendants<NonVisualDrawingProperties>().FirstOrDefault();
                textProp.Name.Value = name;

                //for (int i = 0; i < firstShape; i++)
                //{
                //    var item = firstShape.Shapes[i] as IAutoShape;
