...
module internal ZmqStreamManager =
    ...
    let clientIdentifierToString (clientId: ClientIdentifier) =
        clientId |> map string |> String.concat "-"
    ...
    type StreamManager(fileSpec: IOFileSpec) as this =
        ...
        do fileSpec.StreamManager <- this
        ...
        member x.writeU16 (cri: ClientRequestInfo) (wordOffset: int, value: uint16) =
            x.writeU16s cri ([| wordOffset, value |])
        ...
        member x.writeU32 (cri: ClientRequestInfo) (wordOffset: int, value: uint32) =
            x.writeU32s cri ([| wordOffset, value |])
        ...
        member x.writeU64 (cri: ClientRequestInfo) (wordOffset: int, value: uint64) =
            x.writeU64s cri ([| wordOffset, value |])
...
module internal ZmqBufferManagerExtension =
    ...
    type IOFileSpec with
        ...
        member x.ReadStrings(keys: string[]) =
            ...
            if keys.Length = 0 then
                ...
                let values =
                    [| for key in keys do
                           let values =
                               conn.Query<string>("SELECT [value] FROM [string] WHERE key = @Key", {| Key = key |})
                               |> toArray
        ...
        member x.WriteString(key: string, value: string) =
            ...
            let sqlUpdate =
                "INSERT OR REPLACE INTO [string] ([key], [value]) VALUES (@Key, @Value)"
    ...
    type StreamManager with
        ...
        member x.VerifyIndices(clientRequestInfo: ClientRequestInfo, memoryType: MemoryType, offset: int) =
            let byteOffset =
                ...
                | (MemoryType.Byte | MemoryType.Word | MemoryType.DWord | MemoryType.LWord) ->
                    offset * (int memoryType) / 8
        ...
        member x.VerifyOffsets(clientRequestInfo: ClientRequestInfo, memoryType: MemoryType, offsets: int[]) =
            offsets
            |> iter (fun offset -> x.VerifyIndices(clientRequestInfo, memoryType, offset))
    ...
    type IIOChangeInfo with
        ...
        member x.GetTagNameAndValues() =
            let fs, dataType, offsets, objValues =
                x.IOFileSpec, x.MemoryType, x.Offsets, x.Value
