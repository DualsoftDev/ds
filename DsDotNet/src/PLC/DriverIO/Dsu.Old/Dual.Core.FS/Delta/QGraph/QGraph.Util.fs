...
type IVertexExt =
    ...
    static member generateVertexToModel (v:IVertex) =
        ...
        |> Seq.map(fun dag ->
            { makeSimpleModel (dag.Edges.ToAdjacencyGraph()) with 
                Start = v.StartPort.GetTerminal().ToText() |> Some; 
                Reset = v.ResetPort.GetTerminal().ToText() |> Some; 
                Title = v.Name |> Some 
            } 
        )
    ...
    [<Extension>]
    static member GetGoingStatusExpression (v:IVertex) = v.GoingPort.GetTerminal()
    ...
    static member GetInitialValue (v:IVertex) =
        let sensor = v.SensorPort.GetTag() |> map(mkTerminal) |> Seq.reduce mkAnd
...
module ISelectM =
    let applySelectCondition (v:ISelect) =
        ...
        |> Seq.collect(fun kv ->
            let dag = kv.Value
            let cond = kv.Key
            dag.Vertices |> Seq.iter(fun v -> v.StartPort.PLCFunctions.AddRange(cond)) |> ignore
            dag.Vertices
        )
        |> List.ofSeq
