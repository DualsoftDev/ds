...
module TimerStatementModule =
    ...
    let private createTimerStatement (storages:Storages) (target:HwCPU)  (tParams:TimerCreateParams)  : Statement =
        let ts = TimerStruct.Create(tParams.Type, storages, tParams.Name, tParams.Preset, 0u, RuntimeDS.Param.System.Value, target)
        generateTimerStatement (ts, tParams)
    ...
    type TimerStatement =
        static member CreateTON(tcParams:TCConstructionParams) (target:HwCPU)=
        ...
        static member CreateTOF(tcParams:TCConstructionParams) (target:HwCPU)=
            let {Storages=storages; Name=name; Preset=preset; RungInCondition=rungInCondition; FunctionName=functionName} = tcParams
        ...
        static member CreateAbRTO(tcParams:TCConstructionParams) (target:HwCPU)=
            let {Storages=storages; Name=name; Preset=preset; RungInCondition=rungInCondition; FunctionName=functionName} = tcParams
        ...
        static member CreateTMR(tcParams:TCConstructionParams, resetCondition) (target:HwCPU)=
            let {Storages=storages; Name=name; Preset=preset; RungInCondition=rungInCondition; FunctionName=functionName} = tcParams
            ({  Type=TMR; Name=name; Preset=preset;
                RungConditionIn=Some rungInCondition;
