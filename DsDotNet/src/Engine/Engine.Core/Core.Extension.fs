...
module CoreExtensionModule =
    ...
    type DsSystem with
        ...
        member x.DriveModeLamps     = getLamps(x, DuDriveLamp)
        member x.AutoModeLamps      = getLamps(x, DuAutoLamp)
        member x.ManualModeLamps    = getLamps(x, DuManualLamp)
        member x.StopModeLamps      = getLamps(x, DuStopLamp)
        member x.EmergencyModeLamps = getLamps(x, DuEmergencyLamp)
        member x.TestModeLamps      = getLamps(x, DuTestDriveLamp)
        member x.ReadyModeLamps     = getLamps(x, DuReadyLamp)
