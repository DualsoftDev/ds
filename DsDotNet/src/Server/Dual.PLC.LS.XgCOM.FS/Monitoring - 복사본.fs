...
module MonitorUtil =
    ...
    type LWBatch(lwTagNames:string seq, buffer:byte[], deviceInfos:DeviceInfo seq, tags:XgTag seq) =
    ...
    let batches (conn:DsXgConnection, tagInfos:XgTag array) = 
...
type PLCMonitorEngine() =
    ...
    member private x.MonitorImpl(plcIp, tags:string seq, runSynchronously:bool, delayms:int, bScanWrite:bool) =
        ...
        let batches = batches(conn, tagInfos)
        /// 비동기 computation
        let compu = async {
            let notify (buffer:byte[]) (tagInfo:XgTag) =
            ...
            while true do
                ...
                if bScanWrite
                then 
                    //tagInfos.Filter(fun f-> f.DataType = DataType.Bit)
                    //        .ForEach(fun f->f.WriteValue <- true);
                ...
                //do! Async.Sleep delayms  //배치 사이에만
    ...
    member private x.Monitor(plcIp, tags:string seq, runSynchronously:bool, delayms:int, bScanWrite:bool) =
