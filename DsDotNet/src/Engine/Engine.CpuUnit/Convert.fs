...
module CpuConvertModule =
    ...
    let createReal(realTag:DsTag, graph:DsGraph) =
        ...
        let r2 = realTag.GetTaskEndStatement(cv.Select(fun f->dicM[f]))
        
        //C1 Call 시작조건 Statement 만들기
        let c1 = cv.Select(fun c -> 
                 dicM[c].GetCallStartStatement(c.GetIncomingStart(graph, dicM), realTag))
        //C2 Call 작업완료 Statement 만들기
        let c2 = cv.Select(fun c ->
                 dicM[c].GetCallRelayStatement(c.GetIncomingStart(graph, dicM), [PlcTag.Create(c.Name, false)], realTag))
    ...
    let createRoot(real:Real, graph:DsGraph) =
        ...
        let srcs = real.GetIncomingReset(graph, dicM)
        ...
        let srcs = real.GetIncomingStart(graph, dicM)
