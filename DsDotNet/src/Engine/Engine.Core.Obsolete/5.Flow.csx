...
public class RootFlow : Flow, IParserObject
{
    ...
    public IEnumerable<SegmentBase> RootSegments => ChildVertices.OfType<SegmentBase>().Where(seg => !(seg is ChildFlow));
...
public static class FlowExtension
{
    ...
    public static IEnumerable<Causal> CollectArrow(this Edge edge)
    {
        ...
        foreach (var s in e.Sources)
            yield return new Causal(s, e.Target, new EdgeCausal())
