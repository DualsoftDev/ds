...
|Id| Item | Unit | Example | Desc |   GUI | 
|:---:|:----|:--:|:----|:---|:---|
|SEQ6|Call | `~` |`C = {A ~ B}`<p> `C = {_ ~ B}` <p> empty key is `_` | Upper - Action C calls the action A to execute the DA셋(A ~ B), and indicates the state of action B<p>Lower - Action C just indicates the state of action B without executing<p>`위 - C행위는 A를 실행시키며, B의 상태를 나타냄`<p>`아래 - C행위는 B의 상태만을 나타냄`| ![AAA](./png/Seq6.dio.png)|
...
|SEQ8|Reset Call|`~ ~`| `H = {A,B,C ~ D,E ~ F,G}`|Action H calls the action A, B and C to execute the DAG(A,B,C ~ D,E), or calls the action F, G to reset the state of action D and E<p>And indicates the state of action D and E<p>`F행위는 진행을 위해 A, B, C를 실행시키거나 복귀를 위해 F, G를 실행시킬 수 있고, 이에 따라 변화되는 D, E의 상태를 나타냄`| ![AAA](./png/Seq8.dio.png)|
