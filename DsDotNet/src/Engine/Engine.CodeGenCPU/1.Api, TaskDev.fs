...
type TaskDevManager with
    ...
    member d.TD2_PlanReceive(activeSys:DsSystem) =
        ...
        let off = activeSys._off.Expr
        [|
            for KeyValue(jobFqdn, apiParam) in d.TaskDev.DicTaskTaskDevParamIO do
                let sets =
                    let inParam = apiParam.TaskDevParamIO.InParam
                    match inParam with
                    | Some ip when ip.Type <> DuBOOL ->
    ...
    member d.TD3_PlanOutput(activeSys:DsSystem) =
        ...
        let off = activeSys._off.Expr
        [|
            for KeyValue(jobFqdn, _) in d.TaskDev.DicTaskTaskDevParamIO do
