...
module XGConfigReader  =
    ...
    /// XGK CMD 정보 Source, Destination, Number 등등 타입이 들어있음
    type XgkCMD = {
        Command:  string 
        strStatement:  string //<strStatement>S D N or EN sl ax Num1 Num2</strStatement>
        bySize : int     
    }
    ...
    let getAddressXGK (devices, devicePos, devType, dicMaxDevice:IDictionary<string, int>)=
        let getAddressBit (dHead, devicePos) = 
            ...
            |3  ->  if(dHead = "D" || dHead = "R")
                    then sprintf "%s%03d.%X" devices (devicePos/16) (devicePos%16)
                    else sprintf "%s%03d%X"  devices (devicePos/16) (devicePos%16)
            |4  ->  if(dHead = "D" || dHead = "R")
                    then sprintf "%s%04d.%X" devices (devicePos/16) (devicePos%16)
                    else sprintf "%s%04d%X"  devices (devicePos/16) (devicePos%16)
            |5  ->  if(dHead = "D" || dHead = "R")
                    then sprintf "%s%05d.%X" devices (devicePos/16) (devicePos%16)
                    else sprintf "%s%05d%X"  devices (devicePos/16) (devicePos%16)
            |6  ->  if(dHead = "D" || dHead = "R")
                    then sprintf "%s%06d.%X" devices (devicePos/16) (devicePos%16)
                    else sprintf "%s%06d%X"  devices (devicePos/16) (devicePos%16)
            | _ ->  failwithf "[%s][%d] 예외 디바이스 타입."  dHead (dicMaxDevice.[dHead].ToString().length())

        let getAddressWord (dHead, devicePos) = 
            match dicMaxDevice.[dHead].ToString().length() with 
            |3  -> sprintf "%s%03d"      devices devicePos
            |4  -> sprintf "%s%04d"      devices devicePos
            |5  -> sprintf "%s%05d"      devices devicePos
            |6  -> sprintf "%s%06d"      devices devicePos
            | _ ->  failwithf "[%s][%d] 예외 디바이스 타입."  dHead (dicMaxDevice.[dHead].ToString().length())

        match devType with
        | "BIT"->
            match devices with
            | "S"  ->         sprintf "%s%03d.%02d"    devices (devicePos/100) (devicePos%100)                     
            | "U"  ->         sprintf "%s%02d.%02d.%X" devices (devicePos/16/32) (devicePos/16%32) (devicePos%16)  
        ...
        | "DWORD" -> //word, dword
            match devices with
            | "S"  ->         sprintf "%s%03d"      devices devicePos                    
            | "U"  ->         sprintf "%s%02d.%02d" devices (devicePos/32) (devicePos%32)
            | _  -> getAddressWord(devices, devicePos) 
        | _ ->  failwithf "[%s] 예외 디바이스 타입."  devType 
       
       
    let getAddressXGI (devices, devicePos, devType, dicMaxDevice:IDictionary<string, int>) =
        if(devices = "F")  //F영역만 XGI 처리 
        then
            match devType with
            | "BIT" ->  sprintf "%%%sX%d" devices devicePos
            | "UINT" 
            | "WORD" 
            | "DATE" -> sprintf "%%%sW%d" devices devicePos
            | "DWORD"-> sprintf "%%%sD%d" devices devicePos
            |_ -> failwithf "[%s] 예외 디바이스 타입."  devType 
        else getAddressXGK(devices, devicePos, devType, dicMaxDevice)
    ...
    ///IO Module ID에 해당하는 이름을 XG5000 설정 파일로 부터 가져온다
    ///원본 경로 C:\XG5000\l.kor\CmdDB.mdb 에  XGTCodeDB table을 Xml으로 Export해서 사용
    let readXGKCMDCodeDB () =
        let xdoc = ConfigXml.getXGTCodeDBText().Value |> DsXml.loadXml

        let XGTCodeDB = xdoc.SelectNodes("dataroot/XGTCodeDB")
        XGTCodeDB
        |> XmlExt.ToEnumerables
        |> Seq.map (fun e  -> {
                        Command =  e.SelectSingleNode("Command").InnerText
                        bySize =  Convert.ToInt32(e.SelectSingleNode("bySize").InnerText)
                        strStatement = match e.SelectSingleNode("strStatement") with 
                                        | null -> ""
                                        | v ->  v.InnerText
                            }) 
    ...
    let readConfigFlag (dicMaxDevice, cpuSerise:CpuType) =
        ...
        |> XmlExt.ToEnumerables
        |> Seq.map (fun e  -> 
                         let name =  e.SelectSingleNode("strFlagName").InnerText
                         let devicePos =  Convert.ToInt32(e.SelectSingleNode("nDevicePos").InnerText)
                         let device =  e.SelectSingleNode("strDevice").InnerText
                         let devDataType =  e.SelectSingleNode("strType").InnerText
                         let comment =  dicComment.[Convert.ToInt32(e.SelectSingleNode("nCommentIndex").InnerText)]
                         let address = if(cpuSerise.IsXGI())
                                        then getAddressXGI(device, devicePos, devDataType, dicMaxDevice)
                                        else getAddressXGK(device, devicePos, devDataType, dicMaxDevice)

                         {Name=name; Comment=comment; Device=device; Kind = 0; Type=devDataType; State=0; Address=address; DevicePos=devicePos;}
                           ) 
