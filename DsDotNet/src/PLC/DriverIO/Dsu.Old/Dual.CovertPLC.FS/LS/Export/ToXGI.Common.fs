...
module internal Common =
    ...
    let elementFull type' coordi param tag =
        sprintf "\t\t<Element ElementType=\"%d\" Coordinate=\"%d\" %s>%s</Element>" type' coordi param tag
    /// rung 을 구성하는 element (접점)의 XML 표현 문자열 반환
    let elementBody type' coordi tag = elementFull type' coordi "" tag
    /// rung 을 구성하는 element (접점)의 XML 표현 문자열 반환
    let element type' coordi = elementBody type' coordi ""
    ...
    /// 함수 그리기
    let createFB funcFind func (inst:string) tag x y=
    ...
    let createPA tag x y=
        let c = coord x y
        c, elementFull (int ElementType.VariableMode) c "" tag

    let drawRising (x, y) =
        let results = ResizeArray<int * string>()
        let cellX = getFBCellX x
        let c = coord (cellX) y
        results.Add((c, risingline c))
        results.Add((c, mutiEndLine (x) (cellX-1) y))
        results
    ...
    let vlineDownTo x y n =
        seq {
            if x >= 0 then
                let c = coord x y
                yield c, sprintf "<!-- vlineDownTo %d %d %d -->" x y n
                for n in [0.. n-1] do
                    let c = 2 + coord x (y + n)
                    yield c, vline c
        }
