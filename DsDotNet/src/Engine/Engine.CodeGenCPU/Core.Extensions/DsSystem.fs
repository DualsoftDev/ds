    ...
    type DsSystem with
        ...
        member private x.GenerationCallConditionMemory()  = 
            for condi in x.HWConditions do
                condi.ErrorCondition <- createPlanVar  x.Storages  $"{condi.Name}_err" DuBOOL true condi (int FlowTag.flowStopConditionErrLamp) x
        ...
        member private x.GenerationButtonEmergencyMemory()  = 
            for emg in x.HWButtons.Where(fun f-> f.ButtonType = DuEmergencyBTN) do
                emg.ErrorEmergency <- createPlanVar  x.Storages  $"{emg.Name}_err" DuBOOL true emg (int FlowTag.flowStopEmergencyErrLamp) x
        ...
        member private x.GenerationTaskDevIO() =
            ...
            let vs = x.GetVerticesOfCoins()
            
            for job, devs in jobDevices do
                let coins = vs.GetVerticesOfJobCoins(job)
                for dev in devs do
                if  dev.InAddress <> TextSkip then
                    let inT = createBridgeTag(x.Storages, dev.ApiName, dev.InAddress, (int)ActionTag.ActionIn , BridgeType.Device, x , dev, dev.InParam.DevValueType).Value
                ...
                then
                    if  dev.OutAddress <> TextSkip then
                        let outT = createBridgeTag(x.Storages, dev.ApiName, dev.OutAddress, (int)ActionTag.ActionOut , BridgeType.Device, x , dev, dev.OutParam.DevValueType).Value
