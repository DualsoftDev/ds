...
module ParseOKTest =
    type OK() =
        ...
        [<Test>] member __.``WExternalSegmentCallConfusing`` () = checkParseOK(ParserTest.ExternalSegmentCallConfusing, null)
        [<Test>] member __.``Call3``         () = checkParseOK(ParserTest.Call3, null)
        [<Test>] member __.``XMyOtherFlowCall``() = checkParseOK(ParserTest.MyOtherFlowCall, null)
        [<Test>] member __.``XError``         () = checkParseOK(ParserTest.Error, "Cpu")
        [<Test>] member __.``XExternalSegmentCall`` () = checkParseOK(ParserTest.ExternalSegmentCall, null)
